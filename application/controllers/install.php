...
class Install extends CI_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			$data['install_warnings'][] = 'database.php file not found1';
		}
		...
		else
		{
			...
			$this->load->view('install/warnings', $data);			
	...
	{		
		...
		else
		{
			...
			$data['title'] = 'Welcome to RSS Reader installer';
	...
	function _install_database()
	{
		...
		if ( ! $this->config->config_update(array('encryption_key'=>''.random_string('alnum'), 'base_url'=>$this->base_url)))
		{
		...
		$data['messages'][] = ' we are done! here are default admin login using email: (<h3>'.$this->admin_email.'</h3>) and password: (<h3>'.$this->admin_password.'</h3>). <br> You can change your info in in admin panel. <br>';
